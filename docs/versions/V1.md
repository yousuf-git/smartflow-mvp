...
|---|---|---|
| `id` | string (UUIDv4) | Session id generated by the server. Must be echoed back in every `ack` and `progress` message. |
| `action` | `"START"` | Only value in V1. |
| `litres` | number | Requested volume to dispense. |
...
| Field | Type | Description |
|---|---|---|
| `id` | string (UUIDv4) | Must match the `id` from the `cmd` message exactly. |
| `status` | `"accepted"` \| `"rejected"` | Whether the device will proceed. |
| `reason` | string (optional) | Human-readable explanation when `rejected`. |
...
| `litres` | number | Cumulative litres dispensed so far (monotonically increasing). |
| `status` | `"dispensing"` \| `"complete"` \| `"failed"` | `complete` and `failed` are terminal — no further messages expected after either. |
| `reason` | string (optional) | Included only on `failed`. |
